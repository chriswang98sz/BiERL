...
def warm_up(args,meta_synced_model):
    #mp.set_start_method("spawn")
    def flatten(raw_results, index):
        notflat_results = [result[index] for result in raw_results]
        return notflat_results
    env, _, _, _ = get_env_info("InvertedPendulum-v2")
    synced_model = ESContinuous(env)
    np.random.seed()
    input = [torch.zeros(1, args.n) for _ in range(args.t)]  # make a sequence of length 10
    sigma = meta_synced_model.forward(input)
    for gradient_updates in range(100):
        processes = []
        manager = mp.Manager()
        ...
        meta_state = torch.from_numpy(np.array([(results - np.mean(results)) / (np.std(results) + 1e-8)])).to(torch.float32)
        input[0:-1] = input[1:]
        input[-1] = meta_state
