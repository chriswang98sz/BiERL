...
from Meta_network import MetaES_LSTM
import time
...
def do_rollouts(args, models, random_seeds, return_queue, env, is_negative):

    this_model_returns = 0
    for model in models:
        this_model_return = 0
        state = env.reset()
        ...
        for step in range(args.max_episode_length):
            ...
            if done:
                break
            state = torch.from_numpy(state)
        this_model_returns += this_model_return
    this_model_returns /= len(models)
    return_queue.put((random_seeds, this_model_returns,is_negative))


def perturb_model_single(sigma, model, env):
...
def perturb_meta_model(args, meta_model, random_seed_meta):
    positive_meta_model = MetaES_LSTM(dim_input=args.m)
    negative_meta_model = MetaES_LSTM(dim_input=args.m)
...
def meta_lstm_train(args, synced_model, meta_synced_model, env,input):
    ...
    np.random.seed()
    print("============================================================================================")
    print("Training Meta Learner with LSTM...")
    print("============================================================================================")
    processes = []
    manager = mp.Manager()
    ...
    for j in range(int(args.m / 2)):
        ...
        positive_meta_model,negative_meta_model = perturb_meta_model(args, meta_synced_model,random_seed_meta)
        positive_sigma= positive_meta_model(input)
        negative_sigma= negative_meta_model(input)
        positive_models = []
        negative_models = []
        for _ in range(10):
            positive_model = perturb_model_single(positive_sigma, synced_model, env)
            positive_models += positive_model
        all_seeds.append(random_seed_meta)
        all_models.append(positive_models)
        for _ in range(10):
            negative_model = perturb_model_single(negative_sigma, synced_model, env)
            negative_models += negative_model
        all_seeds.append(random_seed_meta)
        all_models.append(negative_models)
