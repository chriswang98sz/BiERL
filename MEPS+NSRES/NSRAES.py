...
def gradient_update(args, synced_model, returns, novelty, random_seeds, neg_list,
                    num_eps, unperturbed_results, env, sigma,weight):
    def fitness_shaping():
        reward = [x*weight+ y*(1-weight) for x, y in zip(novelty, returns)]
    ...
    print('Average reward: %f\n'
          ...
          (np.mean(returns), np.std(returns), max(returns), min(returns),
           sigma, weight,rank_diag,unperturbed_results))
...
def train_loop_NSRAES(args, meta_synced_model,env):
    ...
    for gradient_updates in range(args.T):
        ...
        synced_model, rank = gradient_update(args, synced_model, results, novelty, seeds,
                                             ...
                                             unperturbed_results, env, sigma,weight)
        ...
        if unperturbed_results > f_max:
            weight = min(1,weight+weight_delta)
            t_max= 0
            f_max = new_unperturbed_results
        else:
            t_max += 1
        if t_max >= t_w:
            weight = max(0, weight - weight_delta)
            t_max = 0
